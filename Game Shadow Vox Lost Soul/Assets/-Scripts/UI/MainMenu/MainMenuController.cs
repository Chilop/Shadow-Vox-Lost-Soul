...
public class MainMenuController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (G_ChangeMenuAnimTrigger) {
            ...
            G_MainMenu.transform.position += new Vector3((G_Inversor*1445) * Time.fixedDeltaTime, 0, 0);
            G_SettingsMenu.transform.position += new Vector3((G_Inversor*1445) * Time.fixedDeltaTime, 0, 0);
            if (G_Timer >= 0.5f) {
                G_Timer = 0;
